...
abstract class FeedWriter
{
	...
	*/
	...
	/**
	 * Genarate the actual RSS/ATOM file as a string
	 *
	 * @access   public
	 * @param    bool  FALSE if the specific feed media type should be send.
	 * @return   array having keys: contentType=>contenttype, xmlcode=>actual xml
	 * @author   giorgio <g.consorti@lynxlab.com>
	 */
	public function generateFeedAsString($useGenericContentType = FALSE) {
		...
		$retArray['xmlcode'] = $this->printHeader(true).
							   $this->printChannels(true).
	...
	* @return   void
	*/
	private function printHeader($return=false)
	{
	...
	* @return   void
	*/
	private function printFooter($return=false)
	{
		$retval = '';
		
		if($this->version == RSS2)
		{
			$retval .=  '</channel>' . PHP_EOL . '</rss>';
		}
		elseif($this->version == RSS1)
		{
			$retval .=  '</rdf:RDF>';
	...
	* @return   void
	*/
	private function printChannels($return=false)
	{
		$retval = '';
		...
		{
			case RSS2:
				$retval .= '<channel>' . PHP_EOL;
				break;
			case RSS1:
				$retval .=  (isset($this->data['ChannelAbout']))? "<channel rdf:about=\"{$this->data['ChannelAbout']}\">" : "<channel rdf:about=\"{$this->channels['link']}\">";
				break;
		}
		...
		foreach ($this->channels as $key => $value)
		{
			if($this->version == ATOM && $key == 'link')
			{
				...
				$retval .=  $this->makeNode($key,'', array('href' => $value));
				//Add the id for ATOM
				$retval .=  $this->makeNode('id', FeedWriter::uuid($value, 'urn:uuid:'));
			}
			else
			{
				$retval .=  $this->makeNode($key, $value);
			}
		...
		{
			$retval .=  "<items>" . PHP_EOL . "<rdf:Seq>" . PHP_EOL;
			foreach ($this->items as $item)
			{
				$thisItems = $item->getElements();
				$retval .=  "<rdf:li resource=\"{$thisItems['link']['content']}\"/>" . PHP_EOL;
			}
			$retval .=  "</rdf:Seq>" . PHP_EOL . "</items>" . PHP_EOL . "</channel>" . PHP_EOL;
		}
	...
	* @return   void
	*/
	private function printItems($return=false)
	{
		$retval = '';
		foreach ($this->items as $item)
		{
			...
			$retval .=  $this->startItem($thisItems['link']['content'],$return);
			
			foreach ($thisItems as $feedItem)
			{
				$retval .=  $this->makeNode($feedItem['name'], $feedItem['content'], $feedItem['attributes']);
			}
			$retval .=  $this->endItem($return);
		}
	...
	* @return   void
	*/
	private function startItem($about = false, $return = false)
	{
		$retval = '';
		...
		else if($this->version == RSS1)
		{
			if($about)
			{
				$retval .=  "<item rdf:about=\"$about\">" . PHP_EOL;
			}
			else
			{
	...
	* @return   void
	*/
	private function endItem($return=false)
	{
		$retval = '';
